...
<center><img src="\assets\img\mess\mess6\6-13.jpg" width="80%" height="80%"></center><br>
    ...
    public int[,] EmptyArray(wordInfo wordInfo) 
    {
        ...
        if (wordInfo.direction)
        {
            for (int i = 0; i < wordInfo.len; i++) //단어의 길이만큼 index를 돌면서
            {
                ...
                {
                    empty[i, 0] = 0;
                    for (int j = wordInfo.x + 1; j < index; j++)
                    {
